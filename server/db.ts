  ...
  try {
    ...
    if (sslCert) {
      ...
      if (fs.existsSync(certPath)) {
        ...
        // للخوادم الخاصة المعروفة والموثوقة، تعطيل التحقق مؤقتاً
        if (connectionString.includes('93.127.142.144') || 
            connectionString.includes('binarjoinanelytic.info')) {
          console.log('🏛️ [SSL] خادم خاص معروف - تعطيل التحقق مؤقتاً لحل مشاكل الشهادات');
          // تعطيل التحقق من الشهادات للخوادم المعروفة
          sslConfig.rejectUnauthorized = false;
          
          console.log('⚠️ [SSL] تم تعطيل التحقق من الشهادات مؤقتاً');
        }
      }
    }
  } catch (error) {
